...
const LoanDetailsShow = ({ loanId, onBack }) => {
    ...
    return (
      <div className="flex justify-center items-center h-full py-20">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-blue-600 font-medium">Loading loan details...</p>
        </div>
      </div>
    );
  }
  ...
  if (!loan) {
    return (
      ...
      >
        ...
        <h2 className="text-xl font-semibold text-gray-700 mt-4">Loan not found</h2>
        <p className="text-gray-500 mt-2">The requested loan details could not be located</p>
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          onClick={onBack}
  ...
  const progressPercentage = Math.min(100, Math.round((loan.totalCollected || 0) / totalAmount * 100));
    ...
    >
      <div className="flex justify-between items-center mb-6">
        <motion.div initial={{ x: -20 }} animate={{ x: 0 }} transition={{ duration: 0.5 }}>
          ...
          <p className="text-gray-600 mt-1 flex items-center">
            ...
            <span>Loan ID: </span>
            <span className="font-mono ml-1 bg-gray-100 px-2 py-0.5 rounded text-sm">{loan._id}</span>
          </p>
        </motion.div>
        <motion.div initial={{ x: 20 }} animate={{ x: 0 }} transition={{ duration: 0.5 }}>
          <motion.button
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <svg className={`h-6 w-6 mr-3 ${loan.totalAmountLeft <= 0 ? "text-blue-500" : "text-green-500"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              {loan.totalAmountLeft <= 0 ? (
            ...
            <div>
              ...
              <div className="text-sm mt-1">
                {loan.totalAmountLeft <= 0 ? "All payments have been made" : "Payments in progress"}
              </div>
            </div>
          </div>
          <div className="text-right">
            ...
            <div>
              ...
              <span className={`font-medium ${loan.totalAmountLeft <= 0 ? "text-blue-600" : "text-green-600"}`}>
        ...
        <div className="mt-4">
          ...
          <div className="w-full bg-gray-200 rounded-full h-2.5">
            <motion.div 
              ...
              className={`h-2.5 rounded-full ${loan.totalAmountLeft <= 0 ? "bg-blue-600" : "bg-green-600"}`}
            ></motion.div>
          </div>
        </div>
      </motion.div>
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between py-1.5 border-b border-gray-200">
                ...
                <div>
                  <span className="font-medium text-gray-800">
                    {loan.paidEmis || 0}/{loan.totalEmis || loan.tenureMonths || loan.tenureDays}
                  </span>
                  {/* Mini progress bar */}
                  <div className="w-16 bg-gray-200 rounded-full h-1.5 ml-2 mt-1.5">
                    <div className="bg-blue-500 h-1.5 rounded-full" style={{ width: `${(loan.paidEmis || 0) / (loan.totalEmis || loan.tenureMonths || loan.tenureDays) * 100}%` }}></div>
                  </div>
                </div>
              </div>
              <div className="flex justify-between py-1.5 border-b border-gray-200">
              ...
              <div className="space-y-3">
                ...
                <div className="flex">
                  ...
                  <span className="text-gray-800">
                    {loan.createdByName} 
                    {loan.createdBy?.role && <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded ml-2">{loan.createdBy?.role}</span>}
                  </span>
                </div>
              </div>
        ...
        >
          ...
          {emiHistoryLoading ? (
            ...
            >
              ...
              <p className="text-gray-500 mt-4">No EMI collection history found for this loan</p>
            </motion.div>
          ) : (
            <motion.div 
              initial={{ y: 20, opacity: 0 }}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          ...
                          {emi.status !== "Paid" && emi.status !== "Pending" && (
                            <svg className="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                          )}
