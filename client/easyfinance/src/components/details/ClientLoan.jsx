...
import { deleteLoan } from "../../services/api.js";
...
const ClientLoans = ({
  ...
  const handleMarkAsComplete = async (e, loanId) => {
    ...
    try {
      ...
      const response = await updateLoanStatus(clientId, loanId, {
        newstatus: "Completed",
      });
      ...
      >
        <motion.div
          ...
          className={`h-16 w-16 border-4 rounded-full ${
            darkMode
              ? "border-t-blue-400 border-blue-700"
  ...
  if (error) {
    return (
      <motion.div
        ...
        } border-l-4 p-5 rounded shadow-md m-6`}
        role="alert"
      >
  ...
  return (
    <motion.div
      ...
      } shadow-lg rounded-xl p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto border transition-colors duration-300`}
    >
  ...
  >
    <span className="flex items-center">
      {processingLoanId === loan._id ? (
        <svg
          className="animate-spin h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1 sm:mr-1.5"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
        >
...
<motion.button
  onClick={(e) => handleDeleteLoan(e, loan._id)}
  whileHover={{ scale: 1.05 }}
  whileTap={{ scale: 0.95 }}
  className={`
    relative overflow-hidden group flex items-center justify-center
