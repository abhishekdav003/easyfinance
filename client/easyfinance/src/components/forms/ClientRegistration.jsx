...
import { User, Phone, MapPin, IndianRupee, Percent, Calendar, Plus, Upload, X, Save, AlertTriangle, XCircle } from "lucide-react";
    ...
    if (formData.loans.length > 0) {
      for (let i = 0; i < formData.loans.length; i++) {
        ...
        if (!loan.loanAmount || !loan.interestRate) {
          ...
          setApiError(`Loan #${i+1} is missing required fields (amount or interest rate)`);
          isValid = false;
          break;
        }
        
        // Numeric validation for loan fields
        if (isNaN(Number(loan.loanAmount)) || isNaN(Number(loan.interestRate))) {
          setApiError(`Loan #${i+1} has invalid amount or interest rate (must be numbers)`);
          isValid = false;
          break;
        }
        ...
        if (loan.emiType === "Monthly" && !loan.tenureMonths && !loan.tenureDays) {
          setApiError(`Loan #${i+1} must have either Tenure Days or Tenure Months`);
          isValid = false;
          break;
        }
        ...
        if (loan.emiType === "Daily" && !loan.tenureDays) {
          setApiError(`Loan #${i+1} must have Tenure Days specified for Daily EMI type`);
          isValid = false;
          break;
        }
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || 
                          (err.message === "Client already exists" ? "Client with this name or phone already exists" : err.message);
      
      setApiError(errorMessage);
      
      if (errorMessage === "Client already exists" || errorMessage.includes("already exists")) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 max-w-4xl mx-auto relative">
      ...
      <div className="flex border-b mb-6">
        <button
          className={`py-2 px-4 font-medium ${activeTab === 'clientInfo' 
        ...
        <button
          className={`py-2 px-4 font-medium ${activeTab === 'loans' 
            ? 'text-blue-600 border-b-2 border-blue-600' 
            : 'text-gray-500 hover:text-gray-700'}`}
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="w-full md:w-1/3 flex flex-col items-center">
                <div className="w-32 h-32 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden border-2 border-dashed border-gray-300 mb-2">
                  {previewImage ? (
                    <img src={previewImage} alt="Client" className="w-full h-full object-cover" />
                  ) : (
              ...
              <div className="w-full md:w-2/3 space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`pl-10 w-full py-3 px-4 border ${nameError ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition`}
                    required
                  />
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`pl-10 w-full py-3 px-4 border ${phoneError ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition`}
                    required
                  />
          ...
          <div className="space-y-6">
            ...
            {showNoLoanWarning && (
              <div className="p-4 bg-yellow-50 border border-yellow-300 rounded-lg">
                <div className="flex items-start">
                  ...
                  <div>
                    <h3 className="font-medium text-yellow-800">Create client without loans?</h3>
                    <p className="text-yellow-700 text-sm mt-1">
                      You're about to create a client without any loans. Click Submit again to confirm, or add a loan below.
                    </p>
                  </div>
                </div>
              </div>
            ...
            {formData.loans.map((loan, i) => (
              <div key={i} className="p-4 border border-gray-200 rounded-lg bg-gray-50 relative">
                ...
                <h3 className="font-medium text-gray-700 mb-4">Loan #{i + 1}</h3>
                ...
                {loan.emiType === "Daily" && !loan.tenureDays && (
                  <p className="mt-2 text-sm text-red-500">Tenure Days is required for Daily EMI type</p>
                )}
                
                {loan.emiType === "Monthly" && !loan.tenureMonths && !loan.tenureDays && (
                  <p className="mt-2 text-sm text-red-500">Either Tenure Months or Days is required for Monthly EMI type</p>
                )}
              </div>
            ))}
